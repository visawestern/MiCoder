...
проверенный маршрут/автотесты; настоящий shell и git repository требуют
отдельного live-QA.
...
Все пункты ✅ вручную прослежены от tab/action до terminal/Git state и
подтверждены повторным полным `swift test`; настоящий shell/repo остаётся
