...
| E16 | `autoImportFromCLI` visible toggle in project row UI | Раздел 8 п.26 |
...
1. **E01** — OpenAI-compatible path attachments (images + files) — TDD.
2. **E06** — call parameters on serve + ACP paths — TDD.
3. **E08** — wire real flows for /plan, /commit, /pr, /review, /context — TDD.
4. **E04** — invoke integrityCheck on project open + restore offer.
5. **E05** — consult `autoImportFromCLI` in the import path.
6. **E03** — wire ProjectDatabaseMigrator into production startup.
7. **E09/E10** — wire executeWithUndo + request_history into real tool operations.
8. **E11** — real MCP health check with callers.
9. **E12** — enforce AccessLevel gate on run_command.
10. **E23/E24** — auto-detect confirmation + overall timeout.
11. **E25** — remove `.modelSettings` case + update tests.
12. **E26/E27/E28** — rebrand strings + Workspaces title + wire/remove dead code.
13. **E13/E14/E15/E16** — read-only fallback, registry export/import, chunked delete, autoImport toggle.
14. **E17/E18/E19** — skill update, dependency resolver, runtime detection.
15. **E07/E22** — per-model cost + cross-project usage aggregation.
16. **E20/E21** — persistent file index + WAL.
...
Remaining fix order: E05 → E03 → E09/E10 → E11 → E12 → E23/E24 → E25 → E26/E27/E28 → E13/E14/E15/E16 → E17/E18/E19 → E07/E22 → E20/E21 → E29/E30/E31.
